...
session_start();
  ...
  $sql1= "SELECT * FROM userreg WHERE user_id='".$userdata1."'";
    ...
    $userdata = mysqli_fetch_array($result1);
        ...
        $vote+=1;
        $updatevote=mysqli_query($con,"UPDATE nationalcard SET votes='$vote'where id='$id'" );
        $updatestatus=mysqli_query($con,"UPDATE userreg SET status=1 where user_id='$userid'" );

        if($updatevote && $updatestatus){
            $userdata['status'] = 1;
          $_SESSION['msg']="success";
            header("location: voting.php?user_id=".$userdata1);
